...
function parsePath(d: string): { start: Point; segments: Segment[] } | null {
  ...
  while (index < tokens.length) {
    ...
    if (upper === 'A') {
      ...
      if (raw[3].length !== 1 || raw[4].length !== 1 || args[3] > 1 || args[4] > 1) {
        return null;
      }
 ...
 */
function arcCenter(from: Point, arc: Arc): { cx: number; cy: number; rx: number; ry: number; delta: number } | null {
  ...
  const factor = (arc.largeArc === arc.sweep ? -1 : 1) * Math.sqrt(Math.max(0, numerator / denominator));
  const cxp = (factor * rx * py) / ry;
  const cyp = (-factor * ry * px) / rx;

  let delta = angleBetween((px - cxp) / rx, (py - cyp) / ry, (-px - cxp) / rx, (-py - cyp) / ry);
 ...
 */
function fitFromArcs(start: Point, arcs: Arc[], tolerance: number): { shape: Ellipse; midpoints: Point[] } | null {
  if (arcs.length < 2) {
    return null;
  }
  ...
  for (const arc of arcs) {
    ...
    if (Math.abs(arc.rx - arcs[0].rx) > tolerance || Math.abs(arc.ry - arcs[0].ry) > tolerance) {
      return null;
    }
    ...
    midpoints.push({
      x: center.cx + cos * center.rx * Math.cos(theta) - sin * center.ry * Math.sin(theta),
      y: center.cy + sin * center.rx * Math.cos(theta) + cos * center.ry * Math.sin(theta),
    });
  ...
  if (Math.abs(from.x - start.x) > tolerance || Math.abs(from.y - start.y) > tolerance) {
    return null;
  }
 ...
 */
function curvesMatch(shape: Ellipse, start: Point, curves: Curve[], tolerance: number): boolean {
  if (curves.length < 3) {
    return false;
  }
  ...
  for (const curve of curves) {
    ...
    if (Math.abs(delta) < 1e-6 || (sum !== 0 && Math.sign(delta) !== Math.sign(sum))) {
      return false;
    }
...
function rewrite(node: { name: string; attributes: Record<string, string> }, name: string, geometry: object): void {
 ...
 *
 * The dicebear CLI carries the same rule, so that a definition written by hand
 * or optimized outside Figma ends up in the shape an export produces. Changes
 * belong in both.
 */
export function convertPathToShape(params: { floatPrecision: number }): CustomPlugin {
    ...
    fn: () => ({
      element: {
        enter: (node) => {
          ...
          if (PATH_BOUND_ATTRIBUTES.some((attribute) => node.attributes[attribute] !== undefined)) {
            return;
          }
          ...
          const ellipse = fitEllipse(start, segments, floatPrecision, matchTolerance, pointTolerance);
          ...
          rewrite(
            ...
            circle ? { cx: ellipse.cx, cy: ellipse.cy, r: ellipse.rx } : ellipse,
          );
        },
      },
    }),
...
function fitEllipse(
  ...
  const withinRadius = (candidate: Ellipse, fitted: Ellipse) =>
    Math.abs(candidate.rx - fitted.rx) <= Math.min(matchTolerance, fitted.rx * 0.05) &&
    Math.abs(candidate.ry - fitted.ry) <= Math.min(matchTolerance, fitted.ry * 0.05);
  ...
  if (segments.every((segment): segment is Arc => segment.kind === 'arc')) {
    ...
    return shortestForm(
      ...
      (candidate) =>
        ...
        nodes.every((node) => ellipseDistance(candidate, node) <= pointTolerance) &&
        fitted.midpoints.every((point) => ellipseDistance(candidate, point) <= 2 * matchTolerance),
    );
  }

  if (!segments.every((segment): segment is Curve => segment.kind === 'curve')) {
    return null;
  }
  ...
  if (Math.abs(last.x - start.x) > matchTolerance || Math.abs(last.y - start.y) > matchTolerance) {
    return null;
  }
