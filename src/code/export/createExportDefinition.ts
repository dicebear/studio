...
export async function createExportDefinition(exportData: Export, warn: (message: string) => void = () => {}) {
  ...
  for (const [colorGroupKey, colorGroupValue] of Object.entries(exportData.colors).sort(byEntryKey)) {
    ...
    const notEqualTo = Object.entries(notEqualToCollection)
      .filter(([key, value]) => {
    ...
    colors[colorGroupKey] = {
      notEqualTo: notEqualTo.length > 0 ? notEqualTo : undefined,
      contrastTo:
  ...
  const definition = JSON.stringify(
    removeEmptyValuesFromObject({
      ...
      $comment:
        'This file was generated by DiceBear Studio for Figma. https://www.figma.com/community/plugin/1005765655729342787',
      meta: {
        license: {
