 ...
 * nothing is cut off, measured in fractions of the layer's box.
 */
export function effectsToFilter(
  ...
  // The default region of ten percent around the box clips large blurs.
  if (padX > 0 || padY > 0) {
    const fractionX = size.width > 0 ? padX / size.width : 0;
    const fractionY = size.height > 0 ? padY / size.height : 0;
